    ...
    if (isNaN(examen3) || examen3 < 0 || examen3 > 5) {
      ...
      if (isNaN(trabajo1) || trabajo1 < 0 || trabajo1 > 5) {
        ...
        if (isNaN(trabajo2) || trabajo2 < 0 || trabajo2 > 5) {
          ...
          let examenFinal = parseInt(
            prompt("Ingrese la nota del examen final")
          );
          if (isNaN(examenFinal) || examenFinal < 0 || examenFinal > 5) {
            console.log("Error digito una nota invalida (0-5)");
          } else {
            let autoEvaluacionEstudiate = parseInt(
              prompt("Ingrese la nota de la Autoevaluacion del alumno")
            );
            if (
              isNaN(autoEvaluacionEstudiate) ||
              autoEvaluacionEstudiate < 0 ||
              autoEvaluacionEstudiate > 5
            ) {
              console.log("Error digito una nota invalida (0-5)");
            } else {
              let autoEvaluacionProfesor = parseInt(
                prompt("Ingrese la nota de la Co-evaluacion del profesor")
              );
              if (
                isNaN(autoEvaluacionProfesor) ||
                autoEvaluacionProfesor < 0 ||
                autoEvaluacionProfesor > 5
              ) {
                console.log("Error digito una nota invalida (0-5)");
              } else {
                ...
                let promedioAutoevaluacion =
                  (autoEvaluacionEstudiate + autoEvaluacionProfesor) / 2;
