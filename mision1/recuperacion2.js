...
do {
  ...
  switch (opcion) {
    case 1:
      ...
      switch (key) {
        case 1:
          cantidadFamiliares = parseFloat(
            prompt("Ingrese la cantidad de familiares")
          );
          while (isNaN(cantidadFamiliares) || cantidadFamiliares <= 0) {
            cantidadFamiliares = parseFloat(
              prompt("Ingrese una cantidad valida de familiares")
            );
          }
          break;
    ...
    case 2:
      for (let index = 0; index < cantidadFamiliares; index++) {
        ...
        while (!isNaN(nombre)) {
          nombre = prompt(
            "Ingrese un nombre valido del familiar " + (index + 1)
          );
        }
        parentesco = prompt(
          "Ingrese el parentesco del familiar " + (index + 1)
        );
        while (!isNaN(parentesco)) {
          parentesco = prompt(
            "Ingrese el parentesco valido del familiar " + (index + 1)
          );
        }
        ...
        while (isNaN(estatura) || estatura <= 0) {
          estatura = parseFloat(
            prompt("Ingrese una estatura valida de " + nombre)
          );
        }
